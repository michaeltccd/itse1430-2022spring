...
In this lab you will update your Character Creator from the previous lab to integrate the Adventure Game design from the first lab and make additional changes to support both features.
...
## Story 1 - Define the Map

Create an `Area` class to represent the rooms from the Adventure Game. Define the areas based upon the original lab design.

### Description

Create an `Area` class to represent a single area/room. At a minimum it will need the following properties.

- An ID to uniquely identify the area.
- A description that can be shown to the user.
- Indicators of which areas are to the North, South, East and West of it.

Define a `World` class that represents the game world you defined previously. It will consist of a set of `Area` instances that represent the world.
At a minimum the class should contain all the areas for the game, properly linked together, and the starting area.
